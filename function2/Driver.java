...
public class Driver {
	...
	public static void main(String[] args) {
		...
		System.out.println( "----------------------");
		final ModelArrayMapper mapper = new ModelArrayMapper();

		JavaRDD<Row> pairRDDReducerRow = pairRDDReducer.map(tuple -> RowFactory.create(mapper.map(tuple)));

		pairRDDReducerRow.foreach(new VoidFunction<Row>() {
            @Override
            public void call(Row tp2) throws Exception {
                System.out.println( tp2.toString());

            }
        });
		SQLContext sqlContext = new SQLContext(sc);
		DataFrame df = sqlContext.createDataFrame(pairRDDReducerRow, mapper.schema());
		df.write().mode(SaveMode.Append).parquet("data/test_table/key=2");
